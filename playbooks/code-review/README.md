...
White-box topic entrypoint for source and configuration review. The root
`playbooks/_catalog.md` routes here, while this module maps a code signal (a grep
hit or scanner lead) to a vulnerability class, an **attack family**, and the
language sink pack to open. The `/audit` orchestrator (`code-audit-loop`) uses
this in P2 triage; `code-auditor` agents use it to pick their sinks.
