#!/usr/bin/env python3
"""Fail closed when the reviewed modern playbook catalog drifts.
...
CATALOG = MODERN / "_catalog.md"
BASELINES = MODERN / "coverage-baselines.json"
REQUIRED = {
    "id",
    "title",
    "family",
...
def main() -> int:
    errors: list[str] = []
    playbooks = sorted(p for p in MODERN.glob("*.md") if p.name != "_catalog.md")
    family_dirs = {p.name for p in FAMILIES.iterdir() if (p / "SKILL.md").is_file()}
    ids: Counter[str] = Counter()
    modern_families: set[str] = set()
    ...
    for path in playbooks:
        ...
        if meta.get("family") not in family_dirs:
            ...
            modern_families.add(meta["family"])
    ...
    catalog_refs = re.findall(r"`([^`/]+\.md)`", catalog_text)
    ref_counts = Counter(catalog_refs)
    expected = {path.name for path in playbooks}
    actual = set(catalog_refs)
    for name in sorted(expected - actual):
        errors.append(f"playbooks/modern/{name}: missing from _catalog.md")
    for name in sorted(actual - expected):
        errors.append(f"playbooks/modern/_catalog.md: unknown playbook {name}")
    for name, count in sorted(ref_counts.items()):
        if count != 1:
            errors.append(f"playbooks/modern/_catalog.md: {name} appears {count} times")
    ...
    for family in sorted(modern_families):
    ...
    print(
        f"modern_playbooks={len(playbooks)} catalog_rows={len(catalog_refs)} "
        f"families={len(modern_families)} baselines={len(baseline_ids)} "
        f"errors={len(errors)}"
    )
    if errors:
        for error in errors:
        ...
        return 1
    print("PASS: modern playbook metadata, sources, family skills, and catalog agree")
    return 0
