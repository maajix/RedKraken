...
MERGES = {
    ...
    "prototype-pollution-clientside-pp.md": (
        "browser-messaging/prototype-pollution.md",
    ...
    "tabnabbing.md": (
        "browser-framing/clickjacking.md",
...
EXPECTED_REVIEWED_SECTIONS = {
    "api/api.md": "## Reviewed consolidation — API discovery",
    "browser-messaging/prototype-pollution.md": "## Reviewed consolidation — client-side gadget confirmation",
    "sql-injection/sqli.md": "## Reviewed consolidation — SQLi scope and remediation",
    "browser-framing/clickjacking.md": "## Reviewed consolidation — opener isolation",
    "browser-script/xss.md": "## Reviewed consolidation — parameter discovery",
...
class WebMergeConsolidationTest(unittest.TestCase):
    def test_sources_are_retired_with_complete_provenance(self) -> None:
        for source, (destination, curated_hash, raw_source, raw_hash) in MERGES.items():
            self.assertFalse(any(PLAYBOOKS.glob(f"*/{source}")), source)
            text = (PLAYBOOKS / destination).read_text(encoding="utf-8")
    ...
    def test_destinations_mark_reviewed_safe_consolidation(self) -> None:
        for destination, heading in EXPECTED_REVIEWED_SECTIONS.items():
            self.assertIn(heading, (PLAYBOOKS / destination).read_text(encoding="utf-8"))

    def test_generated_indexes_drop_retired_slugs(self) -> None:
        catalog = (PLAYBOOKS / "_catalog.md").read_text(encoding="utf-8")
        sources = (PLAYBOOKS / "_sources.tsv").read_text(encoding="utf-8")
