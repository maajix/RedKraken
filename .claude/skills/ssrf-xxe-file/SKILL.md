...
Covers: **SSRF** (+ filter bypasses, DNS rebinding), **XXE** (+ blind/OOB, XXE→LFI/RCE), **LFI / path traversal** (+ PHP filter chain→RCE, log poisoning, wrappers), **File Upload** (+ bypass→webshell), **PDF generators** (HTML/SSRF/LFI). Open the precise playbook via `playbooks/_catalog.md`. Obey `scope-guard` + `tool-preflight`.
...
1. **SSRF.** Load `playbooks/ssrf-url-routing/README.md`. Confirm first
...
3. **LFI / path traversal.** Load `playbooks/file-resolution/README.md`; prove
...
4. **File upload.** Load `playbooks/file-upload/README.md`; map upload →
