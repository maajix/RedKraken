...
**WebSocket/WebTransport**, and **XS-Leaks**. Read the matching browser cards in
`playbooks/modern/` before the imported catalog. Obey `scope-guard` +
`tool-preflight`.
...
7. **Client-side path traversal.** Load `client-side-path-traversal.md`, trace the
...
8. **Browser policy/framing.** Load `browser-policy-framing.md`; preserve raw
...
9. **Storage/offline/templates.** Load `browser-storage-client-templates.md`;
